...
echo "=== users in realm fridaytest (is proton.me user there?) ==="
$K get users -r fridaytest --fields username,email,enabled 2>/dev/null
echo "=== also check poc2 (did it land in the wrong realm?) ==="
$K get users -r poc2 -q username=mircea.craciun@proton.me --fields username,enabled 2>/dev/null || echo "  (not in poc2)"'
