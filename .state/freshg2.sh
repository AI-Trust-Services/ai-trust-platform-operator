...
USER=mircea.craciun@sap.com; ORG=aitrustg2; ACCT=demo
...
kind: Account
metadata: { name: ${ORG} }
spec: { type: org, displayName: "AI Trust G2" }
EOF
}
wait_for 180 5 "org Ready" 'true' 2>/dev/null; sleep 3
oready(){ kc root:orgs get account "$ORG" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null|grep -qx True; }
wait_for 180 5 "org $ORG Ready" oready
sok(){ kc root:orgs get store "$ORG" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null|grep -qx True; }
wait_for 180 5 "store Ready" sok || true; roll
...
wait_for 180 5 "account Ready" aok
...
wait_for 180 5 "binding Bound" b
echo "ORG=$ORG ACCT=$ACCT WS=$WS"
echo "URL=https://$ORG.ai-trust-1.ai-trust.shoot.gardener.cc-one.showroom.apeirora.eu/home/accounts/$ACCT/namespaces/default/aitrust_platform_instances"
