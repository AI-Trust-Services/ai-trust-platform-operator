#!/bin/bash
cd /mnt/c/claude/projects/eu-ai-trust-platform/Standard_AiTrust_MSP
source scripts/lib.sh; load_config
TOK='eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJxTXlJZ000T253THRyRUlHLUI5SzJlaHJzZVFRUUVfVENIRzhjV1VMeHF3In0..Z_du-8AkxNKu9YfqKrWgFZw5e5rb6v8uS0JpUdGfskyaybM4e6jWJw-UeGIRSIhoPnCFT351Rf3rixMts_7qEx1O2F9s14dns_ywOZYoef9HJt6sj3_0QK7f9Uzwk-oBWSqVVibKDS0LvlMOrcOsWK1FyteeH2GpHv0b-mLtC9h3WUvv9z-hloEl9fgA-TTKLq0vwdzXkzOZbyPi1bBqXLeeGH0e5-HlBTnBqxah4EJDu6Xsas_8RR761I0XsnsQxs62pI2A1E9S3A70TmSpRwBkdw9RK_h6pLcDxNpNXFJ4NV8oJIDdV_UVRYGnNWf9l5UXc0TpigLvqfnk4lAxmQ'
HOST=mirceatest3.ai-trust-1.ai-trust.shoot.gardener.cc-one.showroom.apeirora.eu
URL="https://$HOST/gateway/api/clusters/root:orgs:mirceatest3:accounttest/graphql"
# introspect the fields of the v1alpha1 mutation-namespace type
Q='{"query":"query{__schema{mutationType{fields{name type{name kind ofType{name}}}}}}"}'
echo "=== Mutation fields (does createAITrustPlatformInstance / createundefined exist?) ==="
sk -n platform-mesh-system run gq-$RANDOM --rm -i --restart=Never --image=curlimages/curl:8.9.1 --quiet -- \
  curl -sk -X POST "$URL" -H "Authorization: Bearer $TOK" -H 'Content-Type: application/json' --data "$Q" 2>/dev/null > "$STATE/gq.json"
wc -c "$STATE/gq.json"
python3 -c "
import json
d=json.load(open('$STATE/gq.json'))
if 'errors' in d: print('ERRORS:', json.dumps(d['errors'])[:300])
mt=(d.get('data') or {}).get('__schema',{}).get('mutationType')
print('mutationType=', mt.get('name') if mt else None)
for f in (mt or {}).get('fields',[]) or []:
    print(' ', f['name'], '->', (f.get('type') or {}).get('name'))
" 2>&1 | head -40
