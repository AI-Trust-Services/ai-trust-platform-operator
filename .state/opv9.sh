#!/bin/bash
exec > /mnt/c/claude/projects/eu-ai-trust-platform/Standard_AiTrust_MSP/.state/opv9.out 2>&1
...
kubectl -n "$NS" set image "$DEP" '*'=mirceacraciun795/aitrust-msp-operator:v9 2>&1 | grep -avi memcache
