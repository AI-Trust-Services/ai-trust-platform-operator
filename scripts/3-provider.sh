#!/bin/bash
# 3-provider.sh — register the AI Trust provider on the mesh.
#   1) provider workspace root:providers:ai-trust
...
#   3) workload chart on the shoot (operator + syncagent + portal nginx), operator image pinned.
#   4) syncagent hostAlias fix + bind_authenticated.
#   5) wait until the APIExport publishes aitrustplatforminstances.
set -euo pipefail
...
# 3. workload chart on the shoot (operator + syncagent + portal nginx). Embed the kcp-admin kubeconfig,
#    rewriting its server to the provider workspace + :8443 so the syncagent syncs from the right ws.
log "Installing aitrust-msp-app (workload) on the shoot into ns $PROVIDER_NS…"
...
open(p, 'w').write('\n'.join(out) + '\n')
PY
helm --kubeconfig "$SHOOT_KUBECONFIG" upgrade -i aitrust-msp-app "$HERE/../$AITRUST_APP_CHART" \
  ...
  >/dev/null 2>&1 || warn "aitrust-msp-app helm returned non-zero — check: helm --kubeconfig \$SHOOT get -n $PROVIDER_NS"
ok "workload chart installed (operator + syncagent + portal nginx in $PROVIDER_NS)"
...
log "Waiting for APIExport $EXPORT_NAME to publish aitrustplatforminstances…"
...
wait_for 300 10 "APIExport has aitrustplatforminstances" have_res || warn "resource not yet published — check syncagent logs"
ok "AI Trust provider ready"
